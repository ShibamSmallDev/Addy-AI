...
Step 1: requestPowerAction(action) -> mints a single-use, short-lived token
        and tells Addy (via the result string) to ask the user to confirm.
...
# Friendly human labels so Addy's prompt-to-confirm reads naturally.
ACTION_LABEL = {
...
def request_power_action(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Addy calls this first; the returned message instructs her to ask the

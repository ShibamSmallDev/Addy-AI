"""
Windows auto-start management for Addy (V2).

Manages a single registry entry under
    HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\Addy
which points at the silent launcher batch file (start-Addy-silent.bat) located
...
VALUE_NAME = "Addy"
SILENT_LAUNCHER = "start-addy-silent.bat"
...
def _ensure_launcher_exists() -> str:
    """
    Make sure start-Addy-silent.bat exists in the project root.
...
def enable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start enabled. Addy will launch silently on next Windows login.",
        "enabled": True,
        "launcher": launcher,
...
def disable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start disabled. Addy will no longer launch on login.",
        "enabled": False,
    }
...
def get_auto_start_status(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": (
            "Auto-start is ENABLED. Addy launches on Windows login."
            if enabled

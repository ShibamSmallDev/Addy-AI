...
def _resolve_file(path: Optional[str], *, must_exist: bool = False) -> Path:
    ...
    if key in FOLDER_ALIASES:
        p = FOLDER_ALIASES[key]
    else:
        # If the path starts with a folder alias, resolve it and join the rest.
        # Handle both separators so "downloads/file.pdf" and "downloads\\file.pdf" work.
        first = re.split(r"[\\/]", key, maxsplit=1)[0]
        if first in FOLDER_ALIASES:
            rest = str(path)[len(first):].lstrip(os.sep + "/\\")
            p = FOLDER_ALIASES[first] / rest
        else:
...
def _ensure_safe(p: Path, allow_anywhere: bool = False) -> None:
    ...
    for root in SAFE_ROOTS:
        ...
        if real == root_real or real.startswith(root_real + os.sep):
            return
    raise ToolError(
        f"Path '{p}' is outside Addy's safe folders (Desktop, Documents, "
...
@register("readPdf")
def read_pdf(args: Dict[str, Any]) -> Dict[str, Any]:
    """Extract text from a PDF file using pypdf.

    Returns per-page text. Falls back to a clear error if pypdf is not
    installed or the PDF is scanned/image-only (no extractable text).
    """
    path = args.get("path")
    ...
    try:
        ...
        return {
            "result": "Cannot read PDF: the 'pypdf' package is not installed.",
            "path": str(p),
        }
    ...
    try:
        ...
        if not pages:
            return {
                "result": f"PDF has no extractable text (may be scanned/image-only): {p.name}. Use analyzeScreenshot/readScreen OCR on the rendered pages instead.",
                "path": str(p),
                "pages": len(reader.pages),
            }
        return {
            "result": f"{p.name}: {len(reader.pages)} pages, extracted {total} chars.\n\n" + "\n\n".join(pages),
            "path": str(p),
            "pages": len(reader.pages),
            "extracted_chars": total,
        }
    except Exception as e:
        return {"result": f"Failed to read PDF {p.name}: {e}", "path": str(p)}

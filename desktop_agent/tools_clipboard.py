...
def _read_clipboard() -> str:
    try:
        ...
        if text is None:
            raise ToolError("Clipboard does not contain text (e.g. an image or file).")
        return text or ""
    except ToolError:
        raise
    except Exception as e:  # noqa: BLE001
        # Common on Windows when clipboard holds image data (CF_DIB / CF_HDROP)
        err_str = str(e).lower()
        if "image" in err_str or "bitmap" in err_str or "metafile" in err_str or "not text" in err_str:
            raise ToolError("Clipboard contains non-text data (image or file). Cannot read as text.")

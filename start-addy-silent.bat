@echo off
REM ===========================================================================
REM Addy V2 — Silent Auto-Start Launcher
REM ===========================================================================
REM Invoked by the Windows "Run" registry key (HKCU\...\Run\Addy) on login.
...
setlocal
REM Locate Python interpreter with Playwright and FastAPI.
...
for /f "delims=" %%P in ('where py 2^>nul') do ( set "PYTHON_EXE=%%P" & goto :pyfound )
echo [ERROR] Python not found on PATH. Please install Python or set PYTHON_EXE manually.
exit /b 1
:pyfound
...
REM --- 3. Start the Addy web server (Node, port 3000) ------------------------

/**
 * Audio handling utility for Addy Live API Voice stream.
 * Handles:
...
export class AddyAudioSession {
  ...
  private onMousePosition?: (x: number, y: number) => void;
  private onSessionInit?: (sessionId: string) => void;
  ...
  constructor(handlers: {
    ...
    this.onMousePosition = handlers.onMousePosition;
    this.onSessionInit = handlers.onSessionInit;
  }
  ...
  public async connect() {
    ...
    try {
      ...
      this.ws.onopen = async () => {
        console.log("[Addy] Connected to server side WS bridge");
        try {
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (data.type === "status") {
            console.log("[Addy WS Status]:", data.status);
          ...
          // Handle interruption signal (e.g. user talked over Addy)
          ...
          if (data.type === "turnComplete") {
            // Once Addy completes speaking, change visual state back to listening
            setTimeout(() => {
          ...
          if (data.type === "session_init" && data.sessionId) {
            if (this.onSessionInit) {
              this.onSessionInit(data.sessionId);
            }
          }

          // Handle live mouse position tracking
          if (data.type === "mouse_position" && typeof data.x === "number" && typeof data.y === "number") {
            if (this.onMousePosition) {
              this.onMousePosition(data.x, data.y);
            }
          }

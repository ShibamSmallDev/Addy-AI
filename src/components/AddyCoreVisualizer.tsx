...
interface AddyCoreVisualizerProps {
  session: AddyAudioSession | null;
  state: LiveState;
  themeColor: string; // Violet, crimson, emerald, celestial, gold, rose, charcoal
  activeEmotion?: AddyEmotion;
  characterState: "idle" | "thinking" | "talking";
}

export const AddyCoreVisualizer: React.FC<AddyCoreVisualizerProps> = ({
  session,
  state,
  themeColor,
  activeEmotion = "idle",
  ...
  const handleVideoError = (videoName: string) => {
    console.warn(`[Addy Web Video] Failed to load video source for: ${videoName}`);
    setHasError(true);
  };
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      <canvas
        id="Addy-hologram-living-canvas"
        ref={canvasRef}

...
interface SettingsPanelProps {
  ...
  onChange: (patch: Partial<AddySettings>) => void;
  themeColor: string;
}

type SettingsTab = "general" | "voice" | "system" | "personality" | "about";
...
export function SettingsPanel({ isOpen, onClose, settings, onChange, themeColor }: SettingsPanelProps) {
  ...
  // ── Model state ──
  const [availableModels, setAvailableModels] = useState<Record<string, { models: string[]; current: string }>>({});
  ...
  useEffect(() => {
    ...
    fetch("/api/models")
      .then(r => r.json())
      .then((d: Record<string, { models: string[]; current: string }>) => { if (!cancelled) setAvailableModels(d); })
      .catch(() => {})
  ...
  const switchModel = async (provider: string, model: string) => {
    try {
      const res = await fetch("/api/providers/model", {
        method: "POST",
  ...
  // Load custom prompt from backend
  useEffect(() => {
    if (!isOpen || activeTab !== "personality") return;
    let cancelled = false;
    setPromptLoading(true);
    fetch("/api/prompt")
      .then(r => r.json())
      .then(d => { if (!cancelled) setCustomPrompt(d.prompt || ""); })
      .catch(() => {})
  ...
  const tabs: { id: SettingsTab; label: string; icon: any }[] = [
    ...
    { id: "personality", label: "PERSONALITY", icon: PenSquare },
              ...
              {activeTab === "general" && (
                <div className="space-y-4">
                  ...
                  <ToggleRow
                    ...
                    description="Start Addy silently when Windows logs in"
                  ...
                  <ToggleRow
                    label="AUTO-START SCREEN VISION"
                    description="Begin screen sharing automatically on launch (Electron)"
                    checked={settings.autoStartScreenShare}
                    onChange={(v) => onChange({ autoStartScreenShare: v })}
                  />
                  ...
                  {settings.autoStart && (
                    <div className="mt-2 p-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono text-emerald-300/80">
                        Addy will auto-launch on next Windows login.
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <span className="text-[8px] text-slate-500 uppercase font-mono">
                      Say "babe", "hey babe", "addy", or your custom phrase to activate
                    </span>
                  </div>
              ...
              {activeTab === "system" && (
                <div className="space-y-4">
                  ...
                  <div className="pt-2 border-t border-white/5 space-y-2">
                    ...
                    {modelLoading ? (
                      <div className="text-[10px] font-mono text-slate-500">Loading models...</div>
                    ) : (
                      (Object.entries(availableModels) as [string, { models: string[]; current: string }][]).map(([provider, info]) => (
                        <div key={provider} className="flex items-center gap-2">
                          <span className="text-[10px] font-mono text-slate-400 uppercase w-20 shrink-0">{provider}</span>
                          <select
                            value={info.current}
                            onChange={(e) => switchModel(provider, e.target.value)}
                            className="flex-1 px-2 py-1.5 rounded-lg border border-white/10 bg-black/40 text-[10px] font-mono text-white focus:outline-none focus:border-cyan-500/60 cursor-pointer"
                          >
                            {info.models.map((m) => (
                          ...
                          {info.current && (
                            <span className="text-[8px] font-mono text-emerald-400 shrink-0">active</span>
                          )}
                        </div>
                      ))
                    )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-3 rounded-xl border border-cyan-500/15 bg-cyan-500/5 flex items-start gap-2">
                    ...
                    <span className="text-[10px] font-mono text-cyan-300/70 leading-relaxed">
                      These instructions are appended to Addy's system prompt on every voice session. Use it to define extra personality traits, custom rules, or behavioral preferences. Changes apply on next voice connect.
                    </span>
                  </div>
                  {promptLoading ? (
                    ...
                    <>
                      <textarea
                        ...
                        placeholder="e.g. Always speak in a pirate accent. Refer to Shibam as 'Cap'n'. Never use technical jargon."
                        rows={10}
                        className="w-full p-3 rounded-xl border border-white/10 bg-black/40 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/60 resize-none font-mono leading-relaxed"
                      />
                      <div className="flex items-center gap-3">
                        <button
                          onClick={async () => {
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-xl border border-white/5 bg-white/5 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-display text-white">Addy AI Assistant</span>
                    </div>
                  ...
                  <div className="p-3 rounded-xl border border-emerald-500/15 bg-emerald-500/5 flex items-start gap-2">
                    <span className="text-[10px] font-mono text-emerald-300/70 leading-relaxed">
                      Always-on wake word — works even when this tab is in the background.

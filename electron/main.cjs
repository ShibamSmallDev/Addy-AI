/* ===========================================================================
 * Addy — Electron main process (Phase 1)
 * ---------------------------------------------------------------------------
...
function startBackend() {
  ...
  const agentExe = app.isPackaged
    ? path.join(process.resourcesPath, 'agent', 'Addy-agent.exe')
    : path.join(APP_ROOT, 'agent_dist', 'Addy-agent', 'Addy-agent.exe');

  const env = {
    ...process.env,
    NODE_ENV: 'production',
    ELECTRON_RUN_AS_NODE: '1',
    ADJ_LAUNCHED_BY: 'electron',
    ADJ_DATA_DIR: dataDir,
    ADJ_APP_ROOT: APP_ROOT,
  };
  if (fs.existsSync(agentExe)) {
    env.ADJ_AGENT_EXE = agentExe;
  }
  ...
  serverProcess.on('exit', (code, signal) => {
    if (!isQuitting) {
      dialog.showErrorBox(
        'Addy backend stopped',
        `The Addy backend process exited unexpectedly (code ${code}, signal ${signal}).`,
      );
      app.quit();
    }
  });
...
async function bootstrap() {
  app.setAppUserModelId('com.Addy.desktop');

  // Allow the renderer to use getUserMedia with a desktopCapturer source ID.
  // Without this, the chromeMediaSource: 'desktop' constraint is rejected silently.
